...
const LinksPage = () => {
  ...
  return (
    <div className='px-4 lg:px-24 mx-auto'>
      ...
      <div className='bg-primary text-primary-foreground rounded-3xl p-4 space-y-10 mb-20 -mt-12'>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 m-10'>
          {linkCategories.map((category, index) => (
            <div key={index} className='bg-primary p-6'>
              <h2 className='text-2xl font-normal mb-4 text-primary-foreground'>{category.title}</h2>
              <ul className='list-none pl-0'>
                {category.links.map((link, linkIndex) => (
                  <li key={linkIndex} className='mb-2'>
                    <Link href={link.href}>
                      <p className='text-blue-300 hover:text-blue-600 flex items-center'>
